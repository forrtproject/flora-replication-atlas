...
async function fetchData(doi) {
  ...
  try {
    ...
    const original = {
      authors: meta.replications[0].author_o
...
function renderReplications(replications, originalDOI) {
  ...
  replications.forEach((r) => {
    ...
    else if (["success", "failure", "mixed"].includes(rawOutcome))
      normalized = rawOutcome;
  ...
  replications.forEach((rep) => {
    ...
    const pubpeerLink = rep.doi
      ? `https://pubpeer.com/search?q=${rep.doi}`
      : "#";
    const flagLink = `mailto:lukas.roeseler@uni-muenster.de?subject=[Replication Flag] ${originalDOI}&body=I would like to flag a potential issue in the replication record for:%0AOriginal DOI: ${originalDOI}%0AReplication DOI: ${rep.doi}%0AIssue details: [your comment here]`;
